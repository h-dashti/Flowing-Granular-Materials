...
def plot(idir_data,
    ...
    ig3 = np.where(np.isclose(pinffc_expo_table[:,0], gammadot))[0][0]

    fc = fc_table[ig1,1]
    nu = nu_table[ig2,1]
    betaDIVnu = pinffc_expo_table[ig3,1]
    beta = nu * betaDIVnu
    
    #if np.abs(gammadot - 1e-3) < 1e-8:
    #    print(beta)
    #    beta = 0.2
    #    betaDIVnu = beta / nu
    ...
    if show_expo:
        if np.isclose(gammadot, 1e-6): beta += 0.002 
        
        txt = r'$\nu={:.2f}'.format(nu-0.001) + '$'
        ax.text(x=0.8, y=0.22, transform=ax.transAxes,
                s=txt, fontsize=fntsize_info,)
        
        txt = r'$\beta={:.3f}'.format(beta) + '$'
