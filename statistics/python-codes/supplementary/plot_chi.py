...
def plot(idir_data,
    ...
    chimax_expo_table = np.loadtxt(fpath_chimax_expo)

    ig1 = np.where(np.isclose(fc_table[:,0], gammadot))[0][0]
    ig2 = np.where(np.isclose(nu_table[:,0], gammadot))[0][0]
    ig3 = np.where(np.isclose(chimax_expo_table[:,0], gammadot))[0][0]

    fc = fc_table[ig1,1]
    nu = nu_table[ig2,1]
    gammaDIVnu = chimax_expo_table[ig3,1]
    gamma= nu * gammaDIVnu
    ...
    if False:
        xfc2 = xfc - 0.0025 if np.isclose(gammadot, 0.001) else xfc - 0.001
        ax.annotate("$f_c$",
    ...
    if show_expo:
        #if np.isclose(gammadot, 1e-6): gamma += 0.002 
        
        txt = r'$\nu={:.2f}'.format(nu-0.001) + '$'
        ax.text(x=0.8, y=0.22, transform=ax.transAxes,
                s=txt, fontsize=fntsize_info,)
        
        txt = r'$\gamma={:.2f}'.format(gamma) + '$'
        ax.text(x=0.8, y=0.15, transform=ax.transAxes,
                s=txt, fontsize=fntsize_info,)
