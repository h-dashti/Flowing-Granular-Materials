...
def plot_nu(ax,
    ...
    popt, perr = loglog_slope2(x, y, z,
                               #bounds=([0.65, -np.inf], [np.inf, np.inf])
                               )
...
def plot_gamma(ax,
    ...
    txt = r'$\; a^\prime=' + show_error(popt[1], perr[1], 3) + \
        ', \;b^\prime=' + show_error(popt[0], perr[0], 2) + '$'
    ...
    ax.text(x=0.05, y=0.45, transform=ax.transAxes,
            s=txt,
            fontsize=fntsize_info,
                 )
