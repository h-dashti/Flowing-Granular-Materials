...
def get_XY(idir, bname, ix, iy, N_arr, phi_arr, gammadot_arr, catch_fc=False):
	...
	for u, v, w in product(N_arr, phi_arr, gammadot_arr):
		...
		L = get_value(lines, 'L', float)
		n_samples = get_value(lines, 'n_legal_snapshots', int)
		fc_by_chimax = get_value(lines, 'FC_ave_all_samples', float)
	...
	if catch_fc:
		return np.array(data_arr), np.array(params_arr), np.array(fc_by_chimax_arr)
	else:
...
def get_dataHist_rowlines(idir, bname, ix, N_arr, phi_arr, gammadot_arr):
	...
	for u, v, w in product(N_arr, phi_arr, gammadot_arr):
		...
		L = get_value(lines, 'L', float)
		n_samples = get_value(lines, 'n_legal_snapshots', dtype=np.int64)
